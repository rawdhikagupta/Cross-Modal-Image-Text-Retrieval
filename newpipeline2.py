...
if __name__ == '__main__':
    ...
    main('/home/radhika/SOP_SEM2', 'rsicd_precomp')
...
class PrecompDataset(data.Dataset):
    ...
    def __init__(self, data_split, vocab, opt):
        self.vocab = vocab
        self.loc = '/home/radhika/SOP_SEM2/rsicd_precomp/'
        self.img_path = '/home/radhika/SOP_SEM2/RSICD_images/'
        ...
        # local features
        local_features = np.load('/home/radhika/SOP_SEM2/rsicd_local.npy', allow_pickle=True)[()]
        ...
        if data_split != 'train':
            ...
            with open(self.loc + '%s_caps.txt' % data_split, 'rb') as f:
                for line in f:
                    if len(self.captions) < 1024:  # Only add if less than 1000 captions
                        self.captions.append(line.strip())
                    else:
                        break  # Exit the loop once you have 1000 captions
            ...
            with open(self.loc + '%s_filename.txt' % data_split, 'rb') as f:
                for line in f:
                    if len(self.images) < 1024:  # Ensure only 1000 image names are added
                        filename = str(line.strip())[2:-1].split(".")[0] + ".txt"
                        self.local_adj.append(np.array(local_features['adj_matrix'][filename]))
                        self.local_rep.append(np.array(local_features['local_rep'][filename]))
        ...
        print( "Length of captions: ", self.length); 
...
def collate_fn(data):
    ...
    return images, targets, lengths, img_ids, local_rep, local_adj


def get_precomp_loader(data_split, vocab, batch_size=64,
...
def get_loaders(vocab,opt):
    train_loader = get_precomp_loader( 'train', vocab, 64, True, 0,opt)
    # val_loader = get_precomp_loader( 'val', vocab, 64, False, 0,opt)
    return train_loader
...
# print("Checkpoint 1 ******************************************:")
# vocab = Vocabulary()
# vocab_path = '/home/radhika/SOP/rsitmd_splits_vocab.json'  # Update with the correct path
...
class CrossAttention(nn.Module):
    ...
    def forward(self, queries, keys, values):
        ...
        if queries.dim() == 2:
            queries = queries.unsqueeze(0)  # Add a batch dimension
        if keys.dim() == 2:
...
class FusionFFN(nn.Module):
    ...
    def forward(self, vit_features, bert_features):
        # Concatenate the features along the dimension representing feature size
...
class BaseModel(nn.Module):
    ...
    def forward(self, images, captions, local_rep, local_adj):
        ...
        vocab = Vocabulary()
        vocab_path = '/home/radhika/SOP_SEM2/rsitmd_splits_vocab.json'  # Update with the correct path
        vocab = deserialize_vocab(vocab_path)
        ...
        vit_features_with_attention = self.cross_attention(bert_features_reshaped, vit_features_reshaped,vit_features_reshaped)
        bert_features_with_attention = self.cross_attention(vit_features_reshaped, bert_features_reshaped,bert_features_reshaped)
        
        # Assuming vit_features_with_attention and bert_features_with_attention are the attention-processed tensors
        # Initialize the FusionFFN with appropriate sizes, for example:
        input_dim = vit_features_with_attention.size(-1) + bert_features_with_attention.size(-1)  # Sum of feature dimensions
        hidden_dim = 1024  # Example size
        output_dim = 512  # Desired output dimension

        fusion_ffn = FusionFFN(input_dim, hidden_dim, output_dim)
        fused_features = fusion_ffn(vit_features_with_attention, bert_features_with_attention)
...
class ContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, positive_pair, negative_pair):
        ...
        # L2 norm of the positive fused representation
        pos_distance = torch.norm(positive_pair, p=2, dim=1) 
        
        neg_distance = torch.norm(negative_pair, p=2, dim=1)
...
def train(model, data_loader, optimizer, num_epochs):
    ...
    for epoch in range(num_epochs):
        for batch_idx, (images, captions,_, _, local_rep, local_adj) in enumerate(data_loader):
            # The underscore placeholders (_) are for tokens_UNK and image ids, which we don't use here
            ...
            negative_captions = torch.roll(captions, shifts=random.randint(0, 1000), dims=0)
            ...
            if batch_idx % 1 == 0:
                ...
                print(f'Train Epoch: {epoch} \tBatch index: {batch_idx}\tLoss: {loss.item():.6f} \tTime: {current_time}')
                
                with open("logfile2.txt", "a") as log_file:
                    log_message = f'Train Epoch: {epoch} \tLoss: {loss.item():.6f} \tTime: {current_time}\n'
...
# plt.legend()
# plt.grid(True)
# plt.savefig('loss_vs_epoch.png')  # Save the plot as an image
# plt.show()
...
vocab_path = '/home/radhika/SOP_SEM2/rsitmd_splits_vocab.json'  # Update with the correct path
vocab = deserialize_vocab(vocab_path)
train_loader = get_loaders(vocab, opt)

from torch.utils.data import Subset, DataLoader


optimizer = torch.optim.Adam(model.parameters(), lr=0.01)
train(model, train_loader,optimizer,10000)
...
for batch in train_loader:
    ...
    for i in range(min(len(images), 0)):  # Visualising up to 5 images.
        plt.figure()
